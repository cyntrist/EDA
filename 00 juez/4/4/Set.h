...
public:
    ...
    // Unión. Complejidad: casi lineal O(n log n), recorre los elementos de cada set y .add y .contains tienen complejidades logarítmicas.
    Set<T> operator||(const Set& other) const
    {
	    Set newSet(nelems + other.nelems);
        for (int i = 0; i < other.nelems; i++)
            newSet.add(other.array[i]);
	...
	// Intersección. Complejidad: casi lineal O(n log n), recorre los elementos de cada set y .add y .contains tienen complejidades logarítmicas.
    Set<T> operator&&(const Set& other) const
    {
	    int size;
        other.nelems > nelems ? (size = nelems) : (size = other.nelems);
        Set newSet(size);
