...
public:
	...
	Set1(const Set1<T>& other)
	{
		copia(other);
	}

	// operador de asignación
	Set1<T>& operator=(const Set1<T>& other)
	{
		if (this != &other)
		{
			libera();
...
protected:
	...
	void linSearch2(const T& x, int left, int right, bool& found, int& pos) const
	{
	...
	void linSearch(const T& x, int left, int right, bool& found, int& pos) const
	{
...
template <class T>
ostream& operator<<(ostream& out, const Set1<T>& set)
{
	out << "{";
